    ...
    stage('Test infrastructure') {
      ...
      steps {
        withCredentials(bindings: [sshUserPrivateKey(credentialsId: 'ansiblekey', keyFileVariable: 'KEYFILE')]) {
          sh 'testinfra --connection=ansible ansible/roles/webserver/molecule/default/tests/test_default.py  --ansible-inventory=ansible/inventory --ssh-identity-file=${KEYFILE}'
        }
